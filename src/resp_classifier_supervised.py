...
# Extrai coluna com texto do recurso especial e label
texts = df['recurso']
labels = df['num_tema_cadastrado']

# Transforma texto em vetores TF-IDF
vectorizer = TfidfVectorizer()
X = vectorizer.fit_transform(texts)
y = labels

# Configura Cross-Validation, separação em 10 conjuntos . Dados extraidos aleatoriamente do corpus
...
for fold, (train_index, test_index) in enumerate(kf.split(X, y)):
    ...
    classifier = OneVsRestClassifier(LogisticRegression(verbose=1, max_iter=1000))
    ...
    #  6 sugestões para cada texto com as probabilidades correspondentes
    top_suggestions = []
...
with open('classification_supervised_report.txt', 'w') as f:
